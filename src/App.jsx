...
function App() {
  // --- ESTADO DE NAVEGAÇÃO ---
  const [abaAtiva, setAbaAtiva] = useState('gerar-folha'); // 'gerar-folha' | 'rescisao' | 'calculadora'
  ...
  // Máscara para o campo de Horas (HH:MM)
  const handleHorasChange = (e) => {
    let v = e.target.value.replace(/\D/g, ''); // Remove tudo que não é número
    if (v.length > 4) v = v.slice(0, 4); // Limita a 4 dígitos
    if (v.length > 2) {
      v = `${v.slice(0, 2)}:${v.slice(2)}`; // Coloca os dois pontos
    }
    setHorasExtras(v);
  };
  ...
  // --- LÓGICA DE RUBRICAS (GERAR FOLHA) ---
  ...
  // --- LÓGICA DE RESCISÃO (REGRA DOS 15 DIAS) ---
  const calcularMesesValidos = (d1, d2) => {
    let count = 0;
    let dataInicio = new Date(d1);
    let dataFim = new Date(d2);
    ...
    while (d <= dataFim) {
      ...
      let diasTrabalhadosNoMes = (endDay - startDay + 1);
  ...
  const calcularRescisao = () => {
    ...
    setResultadoRescisao({ 
        ...
        multaFgts, 
        total: saldoSalario + decimoTerceiro + feriasComTerco + multaFgts 
    });
  };

  // --- LÓGICA DE HORA EXTRA ---
  const calcularHoraExtra = () => {
    const salario = parseFloat(salarioBaseHE);
    
    if (!salario || !horasExtras || horasExtras.length !== 5) {
        return alert("Preencha o salário e as horas no formato HH:MM corretamente!");
    }

    const [h, m] = horasExtras.split(':').map(Number);
    
    if (m > 59) {
        return alert("Os minutos não podem ser maiores que 59!");
    }

    // Conversão de relógio para decimal
    ...
    const total = horasDecimais * valorHoraExtra;
        ...
        <div>
          ...
          <nav className="space-y-2 px-4">
            <button 
                ...
                className={`w-full text-left px-4 py-3 rounded-lg transition-all flex items-center space-x-3 ${abaAtiva === 'gerar-folha' ? 'bg-slate-800 text-white shadow-lg' : 'hover:bg-slate-800/50 text-slate-400 hover:text-white'}`}
            >
            ...
            <button 
                ...
                className={`w-full text-left px-4 py-3 rounded-lg transition-all flex items-center space-x-3 ${abaAtiva === 'rescisao' ? 'bg-slate-800 text-white shadow-lg' : 'hover:bg-slate-800/50 text-slate-400 hover:text-white'}`}
            >
                <span>Rescisão</span>
            </button>
            ...
            <button 
                ...
                className={`w-full text-left px-4 py-3 rounded-lg transition-all flex items-center space-x-3 ${abaAtiva === 'calculadora' ? 'bg-slate-800 text-white shadow-lg' : 'hover:bg-slate-800/50 text-slate-400 hover:text-white'}`}
            >
                                ...
                                {modoAdicao === 'preenchendo' && (
                                    <div className="bg-white rounded-xl border border-gray-300 p-4 shadow-sm">
                                        <div className="flex justify-between items-center mb-4">
                                            <h4 className="font-medium text-gray-800">Novo {tipoNovoItem === 'provento' ? <span className="text-blue-600">Provento</span> : <span className="text-red-600">Desconto</span>}</h4>
                    ...
                    <div className="bg-white rounded-xl border border-gray-200 p-8 shadow-sm space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <input 
                                    ...
                                    className="border border-gray-300 p-3 rounded-lg w-full focus:ring-2 focus:ring-blue-500 outline-none text-gray-600" 
                                />
                            </div>
                            <div>
                        ...
                        <div className="pt-4">
                            <button 
                                ...
                                className="w-full bg-slate-800 text-white font-bold py-4 rounded-xl hover:bg-slate-700 transition-all flex justify-center items-center space-x-2 text-lg"
                            >
                        ...
                        {resultadoRescisao && (
                            <div className="mt-8 p-6 bg-slate-50 rounded-xl border border-slate-200 text-sm space-y-4">
                                ...
                                <div className="flex justify-between items-center">
                                    ...
                                    <span className="font-medium text-gray-900 text-base">{formatarMoeda(resultadoRescisao.saldoSalario)}</span>
                                </div>
                                <div className="flex justify-between items-center">
                                    <span className="text-gray-600">13º Salário Proporcional:</span>
                                    <span className="font-medium text-gray-900 text-base">{formatarMoeda(resultadoRescisao.decimoTerceiro)}</span>
                                </div>
                                <div className="flex justify-between items-center">
                                    <span className="text-gray-600">Férias + 1/3 Proporcional:</span>
                                    <span className="font-medium text-gray-900 text-base">{formatarMoeda(resultadoRescisao.ferias)}</span>
                                </div>
                                
                                {resultadoRescisao.multaFgts > 0 && (
                                    <div className="flex justify-between items-center pt-2">
                                        <span className="text-red-600 font-medium">Multa FGTS:</span>
                                        <span className="font-bold text-red-600 text-base">{formatarMoeda(resultadoRescisao.multaFgts)}</span>
                                    </div>
                                )}
                                
                                <div className="flex justify-between items-center font-bold text-2xl mt-6 border-t border-gray-200 pt-6 text-slate-900">
                                    <span>Total Líquido:</span>
                                    <span className="text-blue-600">{formatarMoeda(resultadoRescisao.total)}</span>
                                </div>
                            </div>
                        )}
                    </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div>
                                ...
                                <input 
                                    ...
                                    placeholder="Ex: 2000" 
                                    value={salarioBaseHE} 
                                    onChange={(e) => setSalarioBaseHE(e.target.value)} 
                                    className="border border-gray-300 p-3 rounded-lg w-full focus:ring-2 focus:ring-blue-500 outline-none" 
                                />
                            </div>
                            <div>
                                ...
                                <input 
                                    ...
                                    className="border border-gray-300 p-3 rounded-lg w-full focus:ring-2 focus:ring-blue-500 outline-none text-center tracking-widest font-medium" 
                                />
                            </div>
                            <div>
...
<select 
    value={percentualHE} 
    onChange={(e) => setPercentualHE(e.target.value)} 
    className="border border-gray-300 p-3 rounded-lg w-full focus:ring-2 focus:ring-blue-500 outline-none"
>
